...
const createMockFetch = (
  headers: HeadersType = responseHeaders,
  body: object = responseObject,
  url?: string
): FetchType => {
  return async (requestInfo: RequestInfo): Promise<Response> => {
    const uri: string =
      url || typeof requestInfo === 'string' ? (requestInfo as string) : (requestInfo as Request)?.url;
...
export { MockResponse, MockQuery, createMockFetch, executeRequest };
